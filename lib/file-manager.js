...
function copyWWE(session, target) {
    var conf = session.conf,
        src = path.normalize(conf.DEPENDENCIES_BOOTSTRAP + "/wwe"),
        dest = path.normalize(session.sourceDir + "/wwe");

    fs.copySync(src, dest);
}

function copyAuth(session, target) {
    var conf = session.conf,
        src = path.normalize(conf.DEPENDENCIES_BOOTSTRAP + "/auth.txt"),
        dest = path.normalize(session.sourceDir + "/plugins/jnext/");

    if (!path.existsSync(dest)) {
        wrench.mkdirSyncRecursive(dest, "0755");
    }

    fs.copySync(src, dest + "/auth.txt");
}
